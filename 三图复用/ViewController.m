...
- (void)photoClic:(UITapGestureRecognizer *)tap{
    ...
    NSMutableString *muss =  [@"就卡死" mutableCopy];
    for (int i = 0; i<arr3.count; i++) {
        if (i==2) {
            [arr addObject:@[arr3[i],arr3[i+1]]];
        }else if (i==4){
            [arr addObject:@[arr3[i],arr3[i+1],arr3[i+2]]];
        }else
            [arr addObject:[@[arr3[i]] mutableCopy]];
        [muss appendString:@"就卡死"];
        [contents addObject:[muss copy]];
        NSLog(@"%@",arr[i]);
    }
